...
%_$(FLASH_TYPE)_ramfs_fsimage:
	@echo [[$@]]
	cd $(PROJ_ROOT)/../kernel;./scripts/gen_initramfs_list.sh -o $(IMAGEDIR)/$(patsubst %_$(FLASH_TYPE)_ramfs_fsimage,%,$@).ramfs -u 0 -g 0 $(OUTPUTDIR)/rootfs/

kernel_nofsimage:
	@echo [[$@]]
	cp -rvf ../../kernel/arch/arm/boot/uImage.xz $(IMAGEDIR)/$(patsubst %_nofsimage,%,$@)
...
ramdisk_nofsimage:
	@echo [[$@]]
	@ $(PROJ_ROOT)/../kernel/$(CHIP)/scripts/mkimage -A arm -O linux -C none -a 0 -e 0 -n "RAMDISK" -d $($(patsubst %_nofsimage,%,$@)$(RESOUCE)) $(IMAGEDIR)/$(patsubst %_nofsimage,%,$@)
